<?php 
class ModelPaymentLitle extends Model {
  	public function getMethod($address, $total) {
		...
		if ($this->config->get('litle_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('litle_geo_zone_id')) {
			$status = true;
		...
		if ($status) {  
      		$method_data = array( 
				...
				'sort_order' => $this->config->get('litle_sort_order')
      		);
    	}
